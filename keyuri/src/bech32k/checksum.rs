...
impl Checksum {
    /// Create a checksum for the given prefix and data
    pub fn new(prefix: &[u8], data: &[u8]) -> Self {
        let mut p = Polymod::compute(prefix, data);
    ...
    /// Verify this checksum matches the given prefix and data
    pub fn verify(prefix: &[u8], data: &[u8]) -> Result<(), Error> {
        if Polymod::compute(prefix, data).finish() == 1 {
            Ok(())
        } else {
...
mod polymod {
    ...
    impl Polymod {
        pub fn compute(prefix: &[u8], data: &[u8]) -> Self {
