...
impl<'a> Opt<'a> {
    ...
    fn make_action_arg(&self) -> Tokens {
        ...
        let action = match self.action {
            Push(ty) => {
                let act = ty.make_action_type_arg();
